...
// TestConfig_CloudProviderID_AutoGenerated verifies that CloudProviderID is
// absent from JSON when empty (omitempty) and round-trips correctly when set.
func TestConfig_CloudProviderID_AutoGenerated(t *testing.T) {
	tmp := t.TempDir()
	path := filepath.Join(tmp, "config.json")

	// Config without cloudProviderID → empty string after Load.
	require.NoError(t, os.WriteFile(path, []byte(`{"version":"0.1.0"}`), 0600))
	cfg, err := Load(path)
	require.NoError(t, err)
	assert.Equal(t, "", cfg.CloudProviderID,
		"CloudProviderID should be empty when absent from JSON (generation is done by app.go)")

	// Manually assign a GUID (as app.go would on first startup) and verify persistence.
	const testUUID = "{12345678-1234-1234-1234-123456789abc}"
	cfg.CloudProviderID = testUUID
	require.NoError(t, Save(cfg, path))

	loaded, err := Load(path)
	require.NoError(t, err)
	assert.Equal(t, testUUID, loaded.CloudProviderID,
		"CloudProviderID must survive a Save/Load round-trip")
}

// TestConfig_ChunkCacheTTLHours_Default verifies that ChunkCacheTTLHours
// defaults to 24 when absent from the JSON or when explicitly set to 0.
func TestConfig_ChunkCacheTTLHours_Default(t *testing.T) {
	tmp := t.TempDir()
	path := filepath.Join(tmp, "config.json")
	...
	cfg, err := Load(path)
	require.NoError(t, err)
	assert.Equal(t, 24, cfg.ChunkCacheTTLHours,
		"absent chunkCacheTTLHours should default to 24")

	// Explicit zero → also defaults to 24 (zero means "not configured").
	require.NoError(t, os.WriteFile(path, []byte(`{"version":"0.1.0","chunkCacheTTLHours":0}`), 0600))
	cfg, err = Load(path)
	require.NoError(t, err)
	assert.Equal(t, 24, cfg.ChunkCacheTTLHours,
		"chunkCacheTTLHours=0 in JSON should default to 24")

	// Custom value → preserved.
	require.NoError(t, os.WriteFile(path, []byte(`{"version":"0.1.0","chunkCacheTTLHours":48}`), 0600))
	cfg, err = Load(path)
	require.NoError(t, err)
	assert.Equal(t, 48, cfg.ChunkCacheTTLHours,
		"custom chunkCacheTTLHours should be preserved as-is")
}
